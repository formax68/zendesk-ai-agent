...
with gr.Blocks() as ui:
    ...
    def summarize_ticket_internal(ticket_id):
        ...
        from groq_response import GroqResponse
        from zendesk_client import ZendeskClient
        
        if not ticket_id or not ticket_id.strip() or not re.match(r'^\d{6}$', ticket_id.strip()) or int(ticket_id.strip()) <= 500000:
        ...
        response = GroqResponse(user_prompt=prompt, system_prompt=SYSTEM_PROMPT)
        summary = response
